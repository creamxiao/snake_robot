...
public:
  ...
  int getClosestPt(LINK & n){ // get the closest point of sk_cost to the end of this link
      // get the closest point
      int closestPt = -1;
      double dis = 10000;
      for (int a = 0; a < sk_cost.size(); a++){
        bool isBlocked = false;
        double dx = sk_cost[a].x - (n.x + (n.expansion * L + AA * 2) * cos(n.theta));
        double dy = sk_cost[a].y - (n.y + (n.expansion * L + AA * 2) * sin(n.theta));
        double ang = atan2(dy, dx);
        double len = sqrt(dx * dx + dy * dy);

        // check if any wall between the head of link and the point
        for(double i = 1; i <= len; i++){
          if (grey_map.at<uchar>((int)round(n.y + (n.expansion * L + AA * 2) * sin(n.theta) + i * sin(ang)),
          (int)round(n.x + (n.expansion * L + AA * 2) * cos(n.theta) + i * cos(ang))) <= OBSTHRESHOLD){
            isBlocked = true;
            break;
          }
        }
  ...
  double torqueCheck(LINK & n){ // torque about the end of path
    ...
    if (searchMode == 0){
      ...
      for(double i = 0.0; i <= 1; i+=EL){//expanding length: 0, 0.25, 0.5, 0.75, 1.0
        for (int j = -2; j <= 2; j++){
          ...
          double distance = 0.0;
          if (tn.iClosestP_cost > n.iClosestP_cost){ // if they're not the same point
            int s = min(tn.iClosestP_cost, n.iClosestP_cost), g = max(tn.iClosestP_cost, n.iClosestP_cost);
            POINT lastPt = sk_cost[s], thisPt;
            for (int b = s + 1; b <= g; b++){
              thisPt = sk_cost[b];
              distance += sqrt(pow(thisPt.x - lastPt.x, 2) + pow(thisPt.y - lastPt.y, 2));
              lastPt = thisPt;
            }
          }else{
            distance = 0.001;
    ...
    */
    ...
    if (searchMode == 0){
      ...
      int closestIndex = n.iClosestP_cost;

      // if (closestIndex == -1 || C4 == 0.0) return 0; // if there's no closest point, it goes Djistra
      if (closestIndex == -1 ) return 10000.0;
      if (closestIndex < (sk_cost.size() - 1)){ // if the cloeset is not the last point of the path
        // calculate the length from this point to the end
        double acc_length = 0;
        POINT lastPt = sk_cost[closestIndex], thisPt;
        for (int b = closestIndex + 1; b < sk_cost.size(); b++){
          thisPt = sk_cost[b];
          acc_length += sqrt(pow(thisPt.x - lastPt.x, 2) + pow(thisPt.y - lastPt.y, 2));
          lastPt = thisPt;
        }
    ...
    if (searchMode == 0){ // Check if the path is balanced, no need to reach the goal
      if (n.brace){
        ...
        if ((fabs(sin(n.theta) + 1) < INFINITESIMAL_DOUBLE)) return false;
